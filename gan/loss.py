...
def d_loss(real_logits: torch.Tensor, fake_logits: torch.Tensor):
    # loss from real images
    p_real = torch.sigmoid(real_logits)
    ones = torch.ones(len(real_logits)).to(device=real_logits.device)
    loss_real = torch.nn.BCELoss()(p_real, ones)

...
class NeuroEvolution:
    ...
    def mutate(self, parent_list, sigma):
        ...
        for parent in parent_list[0]:
            child = parent + sigma * torch.from_numpy(np.random.normal(
                0, 0.2, parent.shape)).type(torch.FloatTensor).to(self.device)
            ...
            for i in range(self.POPULATION_SIZE):
                if iteration == 0:
                    ...
                    for param in self.weights:
                        # x.append(torch.from_numpy(np.random.randn(
                        #     *param.data.size())).type(torch.FloatTensor).to(self.device))
                        x.append(param.data.to(self.device))
                    n_pop.append([x, 0, i])
                else:
                    p_id = i

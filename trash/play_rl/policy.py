...
class Policy(nn.Module):
    def __init__(
        self, obs_shape, action_space, base=None, base_kwargs=None, model="base"
    ):
...
class CNNBase(NNBase):
    def __init__(
        self, num_inputs, shapes=[], recurrent=False, hidden_size=512, dropout=0
    ):
        super(CNNBase, self).__init__(recurrent, hidden_size, hidden_size)

        self.main = nn.Sequential(
            nn.Conv2d(num_inputs, 128, 3, padding=(3, 1), stride=1),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.Conv2d(128, 64, 3, padding=1, stride=2),
            nn.BatchNorm2d(64),
            nn.ReLU(),
...
class CNNDeep(NNBase):
    def __init__(
        ...
        self.block1 = nn.Sequential(
            nn.Conv2d(num_inputs, 16, 3, padding=(3, 1), stride=1),
            nn.BatchNorm2d(16),
            nn.ReLU(),
        ...
        self.flat = nn.Sequential(
            Flatten(), nn.Linear(32 * 4 * 4, hidden_size), nn.ReLU()
        )
